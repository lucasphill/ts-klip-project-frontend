...
const Navbar = ({ onOpenSidebar }: { onOpenSidebar: () => void }) => {
  ...
  return (
    <header className="z-30 flex h-14 [@media(max-height:600px)]:h-10 shrink-0 items-center justify-between border-b border-[var(--border-subtle)] bg-[var(--bg-panel)] px-4 [@media(max-height:600px)]:px-3">
      <div className="flex items-center gap-3">
        <button
          onClick={onOpenSidebar}
          className="flex h-8 w-8 [@media(max-height:600px)]:h-7 [@media(max-height:600px)]:w-7 items-center justify-center rounded-lg text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-soft)] hover:text-[var(--text-primary)] md:hidden [@media(max-height:600px)]:flex"
          aria-label="Abrir menu lateral"
        >
          <Menu className="h-4 w-4 [@media(max-height:600px)]:h-3.5 [@media(max-height:600px)]:w-3.5" />
        </button>
        <a href="/" className="flex items-center gap-2 rounded-lg p-1 transition-opacity hover:opacity-80">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 [@media(max-height:600px)]:h-6 [@media(max-height:600px)]:w-6" viewBox="0 0 191 191">
            <g>
          ...
          <span className="text-base [@media(max-height:600px)]:text-sm font-semibold tracking-tight text-[var(--text-primary)]">Klip</span>
        </a>
      </div>

      <div className="flex items-center gap-2 [@media(max-height:600px)]:hidden">
